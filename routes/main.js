...
module.exports = function (app, forumData) {
  ...
  app.post('/login',[
    ...
    if (!errors.isEmpty()) {
        ...
        return renderLoginPage(res, req.body, errors.array().map(err => err.msg).join(', '));
    }
  ...
  app.post('/register', [
    ...
    if (!errors.isEmpty()) {
        return renderRegisterPage(res, req.body, errors.array().map(err => err.msg).join(', '));
    }
  ...
  app.post("/addreply", [
      ...
      const errors = validationResult(req);
      if (!errors.isEmpty()) {
          return fetchAndRenderReviewPage(req, res, req.body.reviewId, errors.array().map(err => err.msg).join(', '));
      }
      ...
      db.query(insertReplyQuery, [replyContent, userId, reviewId], (err, result) => {
          if (err) {
              console.error(err);
              return renderReplyPage(res, reviewId, req.body, 'An error occurred while submitting the reply.');
          }
  ...
  app.get('/book/:id/addreview', redirectLogin, async (req, res) => {
    ...
    try {
        ...
        if (bookData) {
            ...
            res.render('addbookreview.ejs', { book: book, forumName: forumData.forumName, errorMessage: null });
        } else {
  ...
  app.post('/submitreview',  [
    ...
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      const book = { id: req.body.bookId, title: req.body.bookTitle, thumbnail: req.body.thumbnail }; 
      return renderSubmitReviewPage(res, book, req.body, errors.array().map(err => err.msg).join(', '));
    }
    ...
    try {
        ...
        const isbn10 = bookData.industryIdentifiers?.find(id => id.type === 'ISBN_10')?.identifier;
        ...
        console.error(error);
        const book = { id: req.body.bookId, title: req.body.bookTitle, thumbnail: req.body.thumbnail }; 
        renderSubmitReviewPage(res, book, req.body, 'An error occurred while submitting the review.');
    }
  });
